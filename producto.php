...
<html> 
	...
	<?php						
		$sql1="select  p.codProducto, p.codCategoria, p.nombre, p.descripcion, p.precio, c.descripcion AS desCategoria
		FROM productos p INNER JOIN categorias c 
		ON c.codCategoria = p.codCategoria
		WHERE codProducto = $codProducto";
					$fila=$con->query($sql1);
						$producto=$fila->fetch_assoc();
						$img=$producto['codProducto'];
						$categoria=$producto['codCategoria'];
						$nombre=$producto['nombre'];
						$descripcion=$producto['descripcion'];
						$precio = $producto['precio'];
						$nombreCategoria = $producto['desCategoria'];
						
					?>
			<div class="content-product">
				<div class="container">
					<ul class="breadcrumb">
					<li><a href=index.php?categoria=<?=$categoria?>><?=$nombreCategoria?></a></li>
					<li><a href="#"><?=$nombre?></a></li>
						</ul>
						<article class="product">
                    <div class="image-box">
                        <img class="logo-main" src=img/productos/<?=$categoria?>/<?=$img?>.jpg alt="placeholder">
                    </div>
                    <div class="description-box">
                        <h1><?=$nombre?></h2>
                        <div class="description">
                            <p><?=$descripcion?></p>
                            <div class="precio"> Precio impuestos incluidos: <strong><?=$precio?> €</strong></div>
                        </div>
						<div class="carrito">
							<form action="anadirCarrito.php" method="post">
							<input type='hidden' name='categoria' value=<?=$img?>>
